...
python manage.py set_sabor_members --year 2011 [--dry-run]  # roster of a Sabor saziv (pre-2015 years; 2007 + 2011 defined)
...
- **Sabor analysis** (`/api/national/sabor-seats/<year>`, `/api/national/sabor-raw/<year>`): every district has a fixed seat count (14 for I-X, 8 across the minority sub-districts) **except diaspora district XI, which is year-dependent** — before 2011 the law converted diaspora votes at the domestic votes-per-seat rate, giving **5 seats in 2007** against the fixed 3 from 2011 on. `diaspora_seats(year)` / `sabor_total_seats(year)` in app.py hold that (`DIASPORA_SEATS_BY_YEAR`), so 2007 correctly reports a **153**-seat Sabor while every other year reports 151; all three D'Hondt sites (`sabor_seat_winner_candidacy_ids`, `sabor_seats`, `sabor_raw`) go through it.
...
- **Seat holders are stored, not always computed**: three sources, in order of preference — a `ParliamentMember` roster (only source for 2007/2011), an `ElectedMandate` row (EU MEPs, where winners ceded seats), else D'Hondt + preferential votes off the results. Rosters and mandate rows are curated in their management commands with the evidence inline, the same convention as `KNOWN_ALIASES`.
