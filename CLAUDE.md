...
python manage.py import_sabor --year 2011             # routed to the separate Sabor2011Importer
...
- **sabor_2011.py** — **2011 only**, dispatched from `import_sabor --year 2011`; kept apart from sabor.py because almost nothing lines up. 2011 predates preferential voting, so DIP published **no candidate columns** for districts I-XI (those years yield ListResult rows only; only district XII names individuals) and **no county columns** — the county is recovered by matching the town name against municipalities already imported from other years (`_resolve_municipality`). All 557 domestic names resolve: repeated place names (OTOK, PRIVLAKA, SVETA NEDELJA each exist in two counties) are disambiguated by which counties vote in that district, learned from the years that *do* record it; district XII files use their own `Izborna jedinica grada / općine` column instead, since they span the whole country. Geo width varies **per file type** (8, 11 or 12 leading columns), so the header is measured at read time rather than hard-coded. Filenames are verbose (`01_I_izborna_jedinica_REDOVITA_BM.csv`) and live under an extra `rezultati/` level. **`Gradovi-opcine` and `DRZAVE` files are skipped** — they are per-municipality / per-country aggregates and importing them would double-count every vote. **Independent lists are all literally named "NEOVISNA LISTA"** and differ only by their `Nositelj liste:`; district I alone runs six, so the holder is appended whenever a base name repeats, giving "NEOVISNA LISTA - IVAN GRUBIŠIĆ" (2 seats) as distinct from Milan Bandić's. Dropping the suffix unconditionally would silently pool their votes. Verified against the official result: 80/47/8/6/6/2/1/1 = 151 seats, SDP-led 41.08% and HDZ 23.50% of the national list vote.
